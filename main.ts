...
console.log( "\n  \t  PART - 02 ")
                      //== 01 == //
let max_value : number = 30;


                     // == 02 == //
let randomNumber : number = Math.floor(Math.random()*max_value) + 1 ;
...
let guesses : number[] = [2,8,11,20,25,30];
...
console.log(`Fianl Counter Value : ${counter}`)
...
for (let collection in myObject) {

    if (myObject.hasOwnProperty(collection))  {

        console.log(`Property Name : ${collection}, Property Value: ${myObject[collection]}`)
    }
}
